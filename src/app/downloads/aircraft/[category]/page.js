...
export default function AircraftCategoryPage({ params }) {
  ...
  const displayIcon = categoryIcons[category] || "✈️";
    ...
    return (
      <main className="min-h-screen bg-black text-white">
        ...
        <div className="text-center py-20">
          ...
          <p className="mt-4 text-gray-400">Loading aircraft...</p>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-gradient-to-b from-black via-zinc-950 to-black text-white">
      ...
      <section className="mx-auto max-w-7xl px-6 py-12">
        <div className="flex items-center gap-3 mb-8">
          <Link href="/downloads/cars/aircraft" className="text-gray-400 hover:text-white">
            ← Back to Aircraft
          </Link>
        ...
        {items.length === 0 ? (
          <div className="text-center py-20">
            ...
            <Link href="/submit" className="mt-4 inline-block text-indigo-400 hover:text-indigo-300">
              Be the first to submit an aircraft!
            </Link>
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {items.map((item) => (
              <Link
                key={item.id}
                href={`/downloads/cars/aircraft/${category}/${item.slug}`}
